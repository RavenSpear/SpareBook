...
export default class MyRelease extends Component {
                ...
                if (JSON.parse(msg).state === 0) {
                    ...
                    for (let num = 0; num < JSON.parse(msg).data.ItemInfoList.length; num++) {
                        result.push({
                            ...
                            state2:(JSON.parse(msg).data.ItemInfoList[num].state===1)?'完成交易':'不卖了',
                        })
                    }
    ...
    endTrade(num){
        if(this.state.searchResult[num].state===1) {
            socketUtil.sendAndReceive(
                "{\"type\":116,\"state\":0,\"data\":{" +
                "\"itemid\":" + this.state.searchResult[num].itemid +
                "}}EOS",
                (msg) => {
                    if(Platform.OS === 'ios'){
                        Alert.alert('交易已完成');
